...
namespace IMServer
{
    ...
    class IMServer
    {
        private readonly TcpListener tcpListener;    // Listens for incoming connections on the port specified in port.txt.
        private readonly Thread listenThread;        // This thread spawns new client threads every time a client connects to the server.
        private readonly UserList userList;          // Maintains a list of all users. Constructed from users.ul
        ...
        // It then starts a new thread which listens for incoming connections.
        {
            ...
            try
            {
                ...
                tcpListener = new TcpListener(IPAddress.Any, incomingPort);
            }
        ...
        // Spawns new threads for each new client connection.
        {
            ...
            while (true)
            {
                TcpClient client = tcpListener.AcceptTcpClient();  // Switch to an async method instead. Track clients in a Collection?

                Thread clientThread = new(new ParameterizedThreadStart(HandleClientComm));   // Find out if this is a safe way to use threading. Check ThreadPool.
        ...
        // It will continue listening to a client until it receives an EXIT REQUEST or until the connection is severed.
        {
            ...
            while (listening)
            {
                ...
                Console.WriteLine("Prefix: " + msgPrefix);
                Console.WriteLine("Content: " + msgContent);
         ...
         */
        {
            switch (msgPrefix)
            {
                ...
                default:
                    Console.WriteLine("Client did not send valid message.");
                    break;
            }
         ...
         */
        {
            Console.WriteLine("Received LOGIN REQUEST from " + ((IPEndPoint)tcpClient.Client.RemoteEndPoint).Address.ToString());
            string clientMsg;
         ...
         */
        {
            string[] splitted = msgContent.Split(['.'], 3);
            ...
            else
            {
                ...
                {
                    Console.WriteLine("User disconnected without calling /exit.");
                    sendTo.Online = false;
                ...
                else
                {
                    ...
                    StreamWriter writeMessage = new(newStream)
                    {
                        AutoFlush = true
                    };
        ...
        static void Main()
        {
            Console.WriteLine("Starting the IM Server...");
            _ = new IMServer();
        }
    }
}
